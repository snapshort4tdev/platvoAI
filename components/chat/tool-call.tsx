...
import ImagePreview from "./tool-image-preview";
...
> = {
  ...
  [ToolTypeEnum.GenerateImage]: (output, input) => {
    return (
      <div className="mb-1.5 mt-1">
        <ImagePreview
          imageUrl={output?.imageUrl || ""}
          prompt={input?.prompt || output?.prompt}
          isLoading={false}
          error={output?.success === false ? output?.message || output?.error : undefined}
        />
      </div>
    );
  },
...
const ToolCall: React.FC<ToolCallProps> = ({
  toolCallId: _toolCallId, // Prefix with _ to indicate intentionally unused
  isLoading,
  type,
  output,
  state,
  ...
  const { text, icon } = getToolStatus(toolName, state, output, t);
